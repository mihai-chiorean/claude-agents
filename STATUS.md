...
- **Research report to review:** [research/vision-constrained-hardware-2026-08-07.md](research/vision-constrained-hardware-2026-08-07.md) — open-vocabulary perception on Jetson Orin (fast vision loop + slow VLM loop), plus Part II applying it to 20-second event description (person–motorcycle tamper vs ride-off; episode pipeline + video-VLM picks + temporal-hallucination defenses). On branch `claude/vision-constrained-hardware-kg15qh`; report-shaped deliverable per Rule 7 (no PR requested). §16 lists the 5 remaining questions blocking a build plan (SKU, camera count, latency, static-camera, retention).
...
- 2026-08-07 — **Researched generic perception on Jetson Orin** (side quest, branch `claude/vision-constrained-hardware-kg15qh`). Deliverable: `research/vision-constrained-hardware-2026-08-07.md` — verdict: fast open-vocabulary vision (YOLOE/NanoOWL/EfficientViT-SAM, 25–95 FPS measured-or-proxied) + slow VLM loop (Qwen3-VL/Florence-2, 0.2–2 Hz) is the correct two-tier design; LLM decode is bandwidth-capped (~0.75×BW÷bytes, confirmed at 70–90% of ceiling on-device) while vision is compute-bound. Ran as 4 parallel general-purpose web-research agents (natural owners vision-engineer/video-analytics-engineer/gpu-engineer aren't spawnable subagent types in remote sessions — same limitation as 2026-06-05). Linear CLI unavailable in the remote container, so no Linear issue/attachment was created — do that on next local session if wanted. Same-day follow-up: scope narrowed to describing ~20 s person–vehicle events → added Part II (§11–16: episode segmentation from tracks, storyboard→video-VLM with schema output, triage tier, reliability evidence — small video VLMs score ≤ chance on temporal order per VinoGround, hence track-timeline-in-prompt + independent heuristic classifier), backed by a fifth research agent on video/temporal models.
